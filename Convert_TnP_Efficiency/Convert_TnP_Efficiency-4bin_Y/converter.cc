    ...
    //for(int entry=111; entry<112; entry++){
	    ...
	    for(int b=0; b<Bsize; b++){
			if(ispp){
				...
				&& Bpt[b]>10 && Bpt[b]<50 && BDTStage1_pt15to50[b]>0.209775
				) 
				if(Bgen[b]==23333){
					...
					hNominal->Fill(fabs(By[b]), _weight);
					//passed and fill
					...
					hScale->Fill(fabs(By[b]), _weight*scale);
					for(int i = 1; i <= 100; i ++){
						...
						hStatTrg[i-1]->Fill(fabs(By[b]), _weight*statTrg);
						hStatMuid[i-1]->Fill(fabs(By[b]), _weight*statMuid);
						hStatSTA[i-1]->Fill(fabs(By[b]), _weight*statSTA);
					}
					...
					hSysHiTrg ->Fill(fabs(By[b]), _weight*sysHiTrg);
					hSysHiTrk ->Fill(fabs(By[b]), _weight*sysHiTrk);
					hSysHiMuid->Fill(fabs(By[b]), _weight*sysHiMuid);
					hSysHiSTA ->Fill(fabs(By[b]), _weight*sysHiSTA);
					hSysLoTrg ->Fill(fabs(By[b]), _weight*sysLoTrg);
					hSysLoTrk ->Fill(fabs(By[b]), _weight*sysLoTrk);
					hSysLoMuid->Fill(fabs(By[b]), _weight*sysLoMuid);
					hSysLoSTA ->Fill(fabs(By[b]), _weight*sysLoSTA);
					hSysBinnedTrg ->Fill(fabs(By[b]), _weight*sysbinnedTrg);
				}
			}
			else{
