    ...
    //for(int entry=111; entry<112; entry++){
	    ...
	    for(int b=0; b<Bsize; b++){
			if(ispp){
				...
				// GA
				&& Btrk1Pt[b]>0.97708799695908488 && Btrk2Pt[b]>0.65705705174287543
				&& abs(Btrk1Eta[b])<2.2997709031344966 && abs(Btrk2Eta[b])<2.2073495278259774
				&& abs(Btrk1Dxy[b]/Btrk1D0Err[b])>-0.38423158068309671 && abs(Btrk2Dxy[b]/Btrk2D0Err[b])>-0.4023172804572468
				&& abs(Btktkmass[b]-1.019455)<0.014078022692914511
				&& BsvpvDistance[b]/BsvpvDisErr[b]>2.4105632972692765
				&& Balpha[b]<2.8539107855538606
				) 
				if(Bgen[b]==23333){
