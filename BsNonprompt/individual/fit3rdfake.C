...
TF1 *f = new TF1(Form("f"),"[0]*([7]*TMath::Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*TMath::Gaus(x,[12],[8])/(sqrt(2*3.14159)*[8]))+ [3]+[4]*x+[5]*x*x + [11]*("+iNP+")");
   if(ispp){
	...
	cp2->Draw("same e");
        ...
        h->Draw("same e");
	f->Draw("same");
	TLegend* leg = new TLegend(0.27,0.66,0.34,0.88,NULL,"brNDC"); 
        leg->SetBorderSize(0);
	leg->SetTextSize(0.03);
	leg->SetTextFont(32);
	leg->SetFillStyle(0);
	leg->AddEntry(h,"Peaking background stack:","p");
        leg->AddEntry(cp2,"3.case: B+ to jpsi K and added extra K","p");
	TLatex * tlatex;
	...
	h->Write();
	f->Write();
	if(ispp) c->SaveAs(Form("plotspp/3rdfitNP_pp.png"));
	else c->SaveAs(Form("plotsPbPb/3rdfitNP_PbPb.png"));
}
