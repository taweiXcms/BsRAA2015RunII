...
//	hempty->SetMaximum(Bnosig->GetMaximum()*1.2);
	...
	leg->AddEntry(h,"Peaking background stack:","p");
        leg->AddEntry(cp0,"1.case: X to Jpsi #pi K","p");
	TLatex * tlatex;
	...
	h->Write();
	f->Write();
	if(ispp) c->SaveAs(Form("plotspp/fitNP1st_pp.png"));
	else c->SaveAs(Form("plotsPbPb/fitNP1st_PbPb.png"));
}
