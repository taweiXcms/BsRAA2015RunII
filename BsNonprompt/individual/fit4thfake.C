...
//        f->SetParameter(1,5.32);
	...
	leg->AddEntry(h,"Peaking background stack:","p");
        leg->AddEntry(cp3,"4.case: X to Jpsi #pi #pi","p");
	TLatex * tlatex;
	...
	h->Write();
	f->Write();
	if(ispp) c->SaveAs(Form("plotspp/4thfitNP_pp.png"));
	else c->SaveAs(Form("plotsPbPb/4thfitNP_PbPb.png"));
}
