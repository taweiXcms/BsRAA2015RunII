...
int BfinderAna_wcuts(
		...
		TString outfile="BJpsiMM_pp_NP_wcuts.root",
		Bool_t isPbPb=0,
		Bool_t REAL=false,
		...
		Bool_t SkimHLTtree=false)
{
	...
	TChain* ch5   = new TChain("BDTStage1_pt15to50","BDTStage1_pt15to50");
	float BDTStage1_pt15to50[MAX_XB];
	ch5->SetBranchAddress("BDTStage1_pt15to50",BDTStage1_pt15to50);
	ch1->Add(infile);
	ch2->Add(infile);
	ch3->Add(infile);
	ch4->Add(infile);
	...
	for(Int_t i=startEntries;i<endEntries;i++)
	{
		...
		for(int j=0;j<BInfo->size;j++){
			...
			if(!(
						Bmass>5.&&Bmass<6. &&
						Bpt>15.&&Bpt<50. &&
						TMath::Abs(By)<2.4&&TMath::Abs(Bmumumass-3.096916)<0.15&&
			...
			if(isPbPb){
				if(!(BDTStage1_pt15to50[j] > 0.303985 && (HLT_HIL1DoubleMu0_v1 || HLT_HIL1DoubleMu0_part1_v1 || HLT_HIL1DoubleMu0_part2_v1 || HLT_HIL1DoubleMu0_part3_v1))) continue;
				//cout<<"Passed PbPb cuts"<<endl;
			}else{
				if(!(BDTStage1_pt15to50[j] > 0.209775 && HLT_HIL1DoubleMu0ForPPRef_v1)) continue;
