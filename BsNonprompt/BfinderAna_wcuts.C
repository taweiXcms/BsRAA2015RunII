...
int BfinderAna_wcuts(
        TString infile="/tmp/cdozen/finder*.root",
        TString outfile="Wcuts_Test2_PbPb.root",
...
//      infile="/data/HeavyFlavourRun2/BfinderRun2/MC/crab_BfinderMC_pp_BJpsiMM_5p02TeV_TuneCUETP8M1_20160613_bPt0jpsiPt0tkPt0p5_Bp.root";
   ...
   TChain *ch1   = new TChain("Bfinder/root","BRoot");
   TChain* ch2   = new TChain("hltanalysis/HltTree","HltAna" );
   TChain* ch3   = new TChain("skimanalysis/HltTree","SkimHlt");
   TChain* ch4   = new TChain("hiEvtAnalyzer/HiTree","HiAna");
   ...
   GenInfo->setbranchadd(ch1);
    ...
    int nbin = (max-min)*50;
    /// Define Histograms
    TH1D* BmassInclusive = new TH1D("BmassInclusive","BmassInclusive", nbin, min, max);                             //All B particles histogram.(Bs,B0.B+...)
...
#include "cut.h"
     /*   if(!isPbPb){
                    if(!(TMath::Abs(By)<2.4&&TMath::Abs(Bmumumass-3.096916)<0.15&&
                        ((abs(Bmu1eta)<1.2 && Bmu1pt>3.5) || (abs(Bmu1eta)>1.2 && 
                        abs(Bmu1eta)<2.1 && Bmu1pt>(5.77-1.8*abs(Bmu1eta))) || 
                        (abs(Bmu1eta)>2.1 && abs(Bmu1eta)<2.4 && Bmu1pt>1.8))&& 
                        ((abs(Bmu2eta)<1.2 && Bmu2pt>3.5) || (abs(Bmu2eta)>1.2 && 
                        abs(Bmu2eta)<2.1 && Bmu2pt>(5.77-1.8*abs(Bmu2eta))) || 
                        (abs(Bmu2eta)>2.1 && abs(Bmu2eta)<2.4 && Bmu2pt>1.8)) && 
                        Bmu1TMOneStationTight && Bmu2TMOneStationTight && Bmu1InPixelLayer > 0 && 
                        (Bmu1InPixelLayer+Bmu1InStripLayer) > 5 && Bmu2InPixelLayer > 0 && 
                        (Bmu2InPixelLayer+Bmu2InStripLayer) > 5 && Bmu1dxyPV< 0.3 &&    
                        Bmu2dxyPV< 0.3 && Bmu1dzPV<20 && Bmu2dzPV<20 && Bmu1isGlobalMuon && 
                        Bmu2isGlobalMuon && Btrk1highPurity&&
                        Bmass>5&&Bmass<6&&Btrk1Pt>0.7&&Btrk2Pt>0.7&&Bchi2cl>1.32e-02&&(Bd0/Bd0Err)>2.&&cos(Bdtheta)>2.60e-01&&Blxy>0.025&&abs(Btktkmass-1.019455)<1.55e-02)) continue; 
              
                         cout<<"Passed pp cuts"<<endl;
        }else{
   ...
   */     
        ...
        if(isPbPb){
                   if(!(TMath::Abs(By)<2.4&&TMath::Abs(Bmumumass-3.096916)<0.15&&
                        ...
                        Bmu2dxyPV< 0.3 && Bmu1dzPV<20 && Bmu2dzPV<20 && Bmu1isGlobalMuon && 
                        Bmu2isGlobalMuon && Btrk1highPurity&&
                        Bmass>5.&&Bmass<6.&&Btrk1Pt>1.5&&Btrk2Pt>1.5&& Bchi2cl>0.1&&(Bd0/Bd0Err)>5.&&cos(Bdtheta)>0.8&&abs(Btktkmass-1.019455)<0.05&&Blxy>0.1)) continue;
                        cout<<"Passed PbPb cuts"<<endl;
        }else{
                cout<<"Passed away"<<endl;
                     }              
            ...
            BmassInclusive->Fill(BInfo->mass[j]);
            bool isBsig = false;
            /*
...
int main(int argc, char *argv[])
{
    ...
    if(argc==3)
    {
        BfinderAna_wcuts(argv[1], argv[2]);
    }
    else
    {
