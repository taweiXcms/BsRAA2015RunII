...
//void Bplusdsigmadpt(TString inputFONLLdat = "pp_Bs_5p03TeV_y2p4", 
void Bsdsigmadpt(TString inputFONLLdat = "pp_Bs_5p03TeV_y2p4", 
TString outputFONLL = "fonllOutput_pp_Bs_y2p4.root",
TString label = "pp")
{
  ...
  TGraphAsymmErrors* gaeSigmaBs=(TGraphAsymmErrors*)gaeSigma->Clone();
  //gaeSigmaBs->SetName("gaeSigmaBs");
  gaeSigmaBs->SetName("gaeSigmaBs");
  gaeSigmaBs->SetFillColor(2);
  gaeSigmaBs->SetFillStyle(3001); 
  gaeSigmaBs->SetTitle(";p_{T}(GeV/c);d#sigma/dp_{T} (B_{s}) (pb GeV-1c)");
  
  for (int i=0;i<gaeSigmaBs->GetN();i++){
    gaeSigmaBs->GetY()[i] *= norm;
    gaeSigmaBs->SetPointEYhigh(i,gaeSigmaBs->GetErrorYhigh(i)*norm);
    gaeSigmaBs->SetPointEYlow(i,gaeSigmaBs->GetErrorYlow(i)*norm); 
  }
  ...
  hempty->GetYaxis()->SetTitle("d#sigma(Bs)/dp_{T}(pb GeV-1c)");
  ...
  gaeSigma->Draw("psame");
  gaeSigmaBs->SetLineWidth(3);
  gaeSigmaBs->SetLineColor(2);
  gaeSigmaBs->Draw("psame");
  ...
  TLegendEntry* ent_gaeSigmaBs=leg->AddEntry(gaeSigmaBs,"Multiplied by Frag. Fraction=0.401","PL");
  ent_gaeSigmaBs->SetTextColor(gaeSigmaBs->GetMarkerColor());
  leg->Draw();
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 4)
    Bsdsigmadpt(argv[1],argv[2],argv[3]);
  return 0;
}
