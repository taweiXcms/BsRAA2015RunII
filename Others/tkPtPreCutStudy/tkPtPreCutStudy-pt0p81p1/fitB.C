...
void fitB(int usePbPb = 0, int fitOnSaved = 0, TString inputdata = "", TString inputmc = "", TString varExp = "", TString trgselection = "",  TString cut = "", TString cutmcgen = "", int isMC = 0, Double_t luminosity = 1., int doweight = 0, TString collsyst = "", TString outputfile = "", TString outplotf = "", TString npfit = "", int doDataCor = 0, Float_t centmin = 0., Float_t centmax = 100., TString inputdatabdt = "", TString inputmcbdt = "")
{
	...
	TFile* infbdt = new TFile(inputdatabdt.Data());
	...
	if(fitOnSaved == 0){
		...
		ntMCbdt = (TTree*)infMCbdt->Get("BDTStage1_pt15to50");
