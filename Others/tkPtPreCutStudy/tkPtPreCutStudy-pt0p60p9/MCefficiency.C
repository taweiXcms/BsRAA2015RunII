...
void MCefficiency(int isPbPb=0, TString inputmc="", TString selmcgen="", TString selmcgenacceptance="", TString cut_recoonly="", TString cut="", TString varExp = "", TString varGenExp = "", TString label="", TString outputfile="", TString outplotf="", int PbPbweight=0, Float_t centmin=0., Float_t centmax=100., TString inputmcbdt = "")
{    
    if(varExp == "Bpt1050"){
        _nBins = nBins1050;
        _ptBins = ptBins1050;
    ...
    TFile* infMCbdt = new TFile(inputmcbdt.Data());
	TTree* ntMC = (TTree*)infMC->Get("ntphi");
	TTree* ntGen = (TTree*)infMC->Get("ntGen");
    TTree* ntMCbdt = (TTree*)infMCbdt->Get("BDTStage1_pt15to50");

	ntMC->AddFriend(ntGen);
	ntMC->AddFriend("ntSkim");
	ntMC->AddFriend("ntHlt");
	ntMC->AddFriend("ntHi");
	ntMC->AddFriend(ntMCbdt);

	ntGen->AddFriend(ntMC);
	ntGen->AddFriend("ntSkim");
	ntGen->AddFriend("ntHlt");
	ntGen->AddFriend("ntHi");
	ntGen->AddFriend(ntMCbdt);
...
{
	if(argc == 16)
		MCefficiency(atoi(argv[1]),argv[2],argv[3],argv[4],argv[5],argv[6],argv[7],argv[8],argv[9],argv[10],argv[11],atoi(argv[12]),atof(argv[13]),atof(argv[14]),argv[15]);
	else
	{
