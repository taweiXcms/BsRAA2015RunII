...
void fitB(int usePbPb = 0, int fitOnSaved = 0, TString inputdata = "", TString inputmc = "", TString varExp = "", TString trgselection = "",  TString cut = "", TString cutmcgen = "", int isMC = 0, Double_t luminosity = 1., int doweight = 0, TString collsyst = "", TString outputfile = "", TString outplotf = "", TString npfit = "", int doDataCor = 0, Float_t centmin = 0., Float_t centmax = 100., TString mva1filedata = "", TString mva1filemc = "", TString mva2filedata = "", TString mva2filemc = "")
{
	...
	if(fitOnSaved == 0){
		...
		//nt->AddFriend("ntHi");
		//nt->AddFriend("ntSkim");
		TFile* infmva1 = new TFile(mva1filedata.Data());
		ntmva1 = (TTree*)infmva1->Get("BDTStage1_pt7to15");
		nt->AddFriend(ntmva1);
		TFile* infmva2 = new TFile(mva2filedata.Data());
		ntmva2 = (TTree*)infmva2->Get("BDTStage1_pt15to50");
		nt->AddFriend(ntmva2);
		...
		TFile* infMCmva1 = new TFile(mva1filemc.Data());
		ntMCmva1 = (TTree*)infMCmva1->Get("BDTStage1_pt7to15");
		ntMC->AddFriend(ntMCmva1);
		TFile* infMCmva2 = new TFile(mva2filemc.Data());
		ntMCmva2 = (TTree*)infMCmva2->Get("BDTStage1_pt15to50");
		ntMC->AddFriend(ntMCmva2);
	}
...
int main(int argc, char *argv[])
{
	...
	{
		fitB(atoi(argv[1]), atoi(argv[2]), argv[3], argv[4], argv[5], argv[6], argv[7], argv[8], atoi(argv[9]), atof(argv[10]), atoi(argv[11]), argv[12], argv[13], argv[14], argv[15], atoi(argv[16]), atof(argv[17]), atof(argv[18]), argv[19], argv[20], argv[21], argv[22]);
	}
	else
	{
