...
void makeoutput(TString infname, TString ofname, string colsys, string mvatype, int varStage){
	...
	TString weightfile = Form("TMVA_Bs-20180223-d4-1550-nominal-weights/TMVA_%s_%s_%.0f_%.0f_varStage%d.weights.xml",mvatype.c_str(),colsys.c_str(),ptmin,ptmax,varStage);
	...
	string varname = Form("%sStage%d_pt%.0fto%.0f_20180223MVA",mvatype.c_str(),varStage,ptmin,ptmax);
