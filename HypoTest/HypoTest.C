...
void HypoTest(){
	...
	// dont let profile poi related parameters, otherwise its just a random value
	RooArgSet meanArgSet("meanArgSet");
	meanArgSet.add(*ws->var(Form("mean%d",_count)));
	poiAndGlobalObs.add( meanArgSet );
	...
	// 1st was of getting best fit value
	pProfile->getVal(); // this will do fit and set nuisance parameters to profiled values
	// 2nd way of getting best fit value
...
//	((RooRealVar *)poi.first())->setConstant(kFALSE);
	...
	frame->Draw();
	c->SaveAs(Form("PDF_%s_%s.pdf",coll.Data(),ptbin.Data()));
	//return;
	...
	//RooArgSet* arg = bModel->GetPdf()->getVariables();
	...
	c->SaveAs(Form("result_%s_%s_%d.pdf",coll.Data(),ptbin.Data(),ntoys));
}
