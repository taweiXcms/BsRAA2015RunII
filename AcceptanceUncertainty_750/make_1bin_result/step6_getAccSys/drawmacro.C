...
const int nBins = 2;
double ptbin[nBins+1] = {7,15,50};
double Rat[nBins];
double RatErr[nBins];
const int nBinsy=1;
double ybin[nBinsy+1] = {0.0,2.4};
double Ratylab[nBinsy];
double RatErrylab[nBinsy];

string label;

void drawmacro(bool ispp=1){
	if(ispp) label = "pp";
	else label = "PbPb";
	...
	TH1D* ha1gaus=new TH1D("ha1gaus",";[0] ([0]p_{T}+[1]);Entries",200,-0.04,0.02);
	TH1D* ha2gaus=new TH1D("ha2gaus",";[1] ([0]p_{T}+[1]);Entries",260,0.4,1.8);
	TH1D* hay1gaus=new TH1D("hay1gaus",";[0] ([0](y)^{2}+[1]);Entries",200,-0.20,0.28);
	TH1D* hay2gaus=new TH1D("hay2gaus",";[1] ([0](y)^{2}+[1]);Entries",250,0.5,1.3);
	...
	/// plot acceptance variation from step5 (pull distribution)
	for (int i=0;i<nBins;i++){
...
//		TCanvas* c1=new TCanvas("c1","");
		...
		p21->Draw("same");
		leg1->Draw("same");
		c1->SaveAs(Form("Bs_ydep_%i_%s.pdf",i,label.c_str()));
	}

	/// plot acceptance variation from step5
	for (int i=0;i<nBins;i++){
		printf("pt bins %d\n", i);
...
//		TCanvas* c1=new TCanvas("c1","");
		...
		float rms = (hf11pt[i]->GetRMS()+hf21pt[i]->GetRMS())/2;
		float loedge = mean-rms*10;
		float hiedge = mean+rms*10;
...
//		TCanvas* c1=new TCanvas("c1","");
		float mean = (hf11y[i]->GetMean()+hf21y[i]->GetMean())/2;
		float rms = (hf11y[i]->GetRMS()+hf21y[i]->GetRMS())/2;
		float loedge = mean-rms*10;
		float hiedge = mean+rms*10;
		TH1D* hempty = new TH1D("","",10,loedge,hiedge);
...
//	TCanvas* c1=new TCanvas("c1","");
	...
	ha2gaus->Draw("");
	leg1->Draw("same");
	c1->SaveAs(Form("Bs_ha2comp_%s.pdf",label.c_str()));
	leg1->Clear();
	...
	hay1gaus->Draw("");
	leg1->Draw("same");
	c1->SaveAs(Form("Bs_hay1comp_%s.pdf",label.c_str()));
	leg1->Clear();
	...
	hay2gaus->Draw("");
	leg1->Draw("same");
	c1->SaveAs(Form("Bs_hay2comp_%s.pdf",label.c_str()));

	//// plot the toy ratio of bin 1 from step3
	leg1->Clear();
	...
	htest11->Draw("");
	leg1->Draw("same");
	c1->SaveAs(Form("Bs_toyratio_ptbin1_%s.pdf",label.c_str()));
	leg1->Clear();
	...
	htest21->Draw("");
	leg1->Draw("same");
	c1->SaveAs(Form("Bs_toyratioy_bin1_%s.pdf",label.c_str()));
