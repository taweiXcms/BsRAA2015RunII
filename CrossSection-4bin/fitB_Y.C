...
using namespace std;
void fitB_Y(int usePbPb=0, TString inputdata="" , TString inputmc="", TString trgselection="",  TString cut="", TString cutmcgen="", int isMC=0, Double_t luminosity=1., int doweight=0, TString collsyst="", TString outputfile="", TString npfit="", int doDataCor = 0, Float_t centmin=0., Float_t centmax=100.)
{
	...
	ntGen->Project("hPtGen","abs(Gy)",TCut(weightgen)*(TCut(selmcgen.Data())));
...
TF1 *fit(TTree *nt, TTree *ntMC, Double_t ptmin, Double_t ptmax, int isMC,bool isPbPb,TF1* &total,Float_t centmin, Float_t centmax, TString npfit)
{
	...
	nt->Project(Form("htest-%d",count),"Bmass",Form("%s&&%s&&abs(By)>%f&&abs(By)<%f)*(1/%s)",sideband.Data(),seldata.Data(),ptmin,ptmax,weightdata.Data()));
    ...
    tex = new TLatex(0.488,0.82,Form("%.1f < |y| < %.1f GeV/c",ptmin,ptmax));
	tex->SetNDC();
	tex->SetTextFont(42);
    ...
    tex = new TLatex(0.488,0.75,"10. < p_{T} < 50.");
	tex->SetNDC();
	tex->SetTextFont(42);
	tex->SetTextSize(0.06);
	tex->SetLineWidth(2);
	...
	if(isPbPb && isMC==0) 
		c->SaveAs(Form("plotFits/data_PbPb_%.1f_%.1f%s_Y.pdf",ptmin,ptmax,_postfix.Data()));
	else if(isPbPb && isMC==1) 
		c->SaveAs(Form("plotFits/mc_PbPb_%.1f_%.1f%s_Y.pdf",ptmin,ptmax,_postfix.Data()));
	else if(!isPbPb && isMC==0) 
		c->SaveAs(Form("plotFits/data_pp_%.1f_%.1f%s_Y.pdf",ptmin,ptmax,_postfix.Data()));
	else 
		c->SaveAs(Form("plotFits/mc_pp_%.1f_%.1f%s_Y.pdf",ptmin,ptmax,_postfix.Data()));
...
int main(int argc, char *argv[])
{
	if(argc==16)
	{
		fitB_Y(atoi(argv[1]),argv[2], argv[3], argv[4], argv[5], argv[6], atoi(argv[7]), atof(argv[8]), atoi(argv[9]),argv[10],argv[11],argv[12],atoi(argv[13]),atof(argv[14]),atof(argv[15]));
		return 0;
	}
	else if(argc==14)
	{
		fitB_Y(atoi(argv[1]), argv[2], argv[3], argv[4], argv[5], argv[6], atoi(argv[7]), atof(argv[8]), atoi(argv[9]),argv[10],argv[11], argv[12], atoi(argv[13]));
		return 0;
	}
	else
	{
