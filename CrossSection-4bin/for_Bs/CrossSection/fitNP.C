...
void fitNP(int usePbPb=0, TString inputdata="/afs/cern.ch/work/c/cdozen/UP_BFINDER/CMSSW_7_5_8_patch3/src/Bfinder/Bfinder/Bntuple/looper/NPfiles/loop_BJpsiMM_pp_NP.root" , 
...
TString cutmcgen="TMath::Abs(Gy)<2.4&&GisSignal>0&&Bgen!=23333", 
int isMC=1, Double_t luminosity=1., int doweight=0, 
TString collsyst="PP", 
TString outputfile="ROOTfiles/mytest.root", Float_t centmin=0., Float_t centmax=0.)
{
...
TF1 *fit(TTree *nt, TTree *ntMC, Double_t ptmin, Double_t ptmax, int isMC,bool isPbPb,TF1* &total,Float_t centmin, Float_t centmax)
{
	...
	TH1D* h = new TH1D(Form("h%d",count),"",nbinsmasshisto,minhisto,maxhisto);
        //TF1* f = new TF1(Form("f%d",count),"Gaus(x,5.36800e+00,5.77122e-02)/(sqrt(2*3.14159)*abs(5.77122e-02))");
	...
	TF1* f = new TF1(Form("f%d",count),"[0]*TMath::Erf((x-[1])/[2]) + [0] + [9]+[10]*x+[11]*x*x  ");
