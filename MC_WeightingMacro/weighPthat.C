...
Bool_t isInsidebin(Float_t xpthat, Float_t maxBgenpt, Int_t i)
{
  ...
  if(i<(nBins-1)
	&& (xpthat>pthatBin[i]&&maxBgenpt>ptBin[i])
    &&!(xpthat>pthatBin[i+1]&&maxBgenpt>ptBin[i+1])) return true;
  else if(i==(nBins-1)&&xpthat>pthatBin[i]&&maxBgenpt>ptBin[i]) return true;
  else return false;
}
