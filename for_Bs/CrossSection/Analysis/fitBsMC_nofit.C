...
//TString inputmc="/afs/cern.ch/work/c/cdozen/BFINDER/updatedversion/CMSSW_7_5_8_patch3/src/Bfinder/Bfinder/Bntuple_original/BsMC_2015_merged_pp.root";//official MC
//pp mc//
...
//pthat10///TString inputmc="/afs/cern.ch/work/c/cdozen/UP_BFINDER/CMSSW_7_5_8_patch3/src/Bfinder/Bfinder/Bntuple/loop_BsMC_PbPb_pthat10.root";
//pthat5//
 TString inputmc="/afs/cern.ch/work/c/cdozen/UP_BFINDER/CMSSW_7_5_8_patch3/src/Bfinder/Bfinder/Bntuple/loop_BsMC_PbPb_pthat5.root";
...
//const int nBins = 1; Double_t ptBins[nBins+1] = {5.,60.};// larger then 60 we don't have any entries
TString weight = "1"; 
//v1 cuts
TString cut = "TMath::Abs(By)<2.4&&abs(Bmumumass-3.09691 6)<0.15&&Bmass>5.&&Bmass<6.&&Btrk1Pt>1&&Btrk2Pt>1&& Bchi2cl>0.1&&(Bd0/Bd0Err)>5.&&cos(Bdtheta)>0.8&&Bmu1pt>1&&Bmu2pt>1&&TMath::Abs(Btktkmass-1.019455)<0.05&&Blxy>0.1"; 
...
//TString selmc=Form("abs(By)<2.4&&%s",cut.Data());
//TString selmc=Form("%s",cut.Data());
//TString selmcgen="TMath::Abs(Gy)<2.4&&abs(GpdgId)==531&&GisSignal>0";
//TString selmcgenacceptance="TMath::Abs(Gy)<2.4&&abs(GpdgId)==531&&GisSignal>0";
...
void fitBsMC_nofit(TString infname="",Bool_t doweight = false)
{
...
// if (doweight==0) weight="1";
  ...
  TLatex* texlumi = new TLatex(0.9,0.92," 27.87 pb^{-1} (5.03 TeV PbPb)");
  texlumi->SetNDC();
...
TH1D *hPtGenAll = new TH1D("hPtGenAll","",nBins,ptBins);
    ...
    for (int i=0;i<nBins;i++)
   {
      ...
      //double yield = f->Integral(5,6)/0.025;
      double yieldErr = f->Integral(5,6)/0.04*f->GetParError(0)/f->GetParameter(0);
      //double yieldErr = f->Integral(5,6)/0.025*f->GetParError(0)/f->GetParameter(0);
   ...
   //c->SaveAs(Form("ResultBs/hPtResult_PbPb.pdf"));
   //c->SaveAs(Form("ResultBs/3K_hPtResult_PbPb.pdf"));
   //pthat0//c->SaveAs(Form("ResultBs/hPtResult_PbPbi_MC.pdf"));
   c->SaveAs(Form("ResultBs/hPtResult_PbPb_MC_pthat5.pdf"));
   //c->SaveAs(Form("ResultBs/hPtResult_v3.pdf"));

   ntMC->Project("hPtMC","Bpt",TCut(weight)*(TCut(selmc.Data()))&&"(Bgen==23333)");
   //ntMC->Project("hRecoTruth","Bpt",TCut(cut_recoonly.Data())&&"(Bgen==2200)");
   ntMC->Project("hRecoTruth","Bpt",TCut(seldata_2y.Data())&&"(Bgen==23333)");
   //nt->Project("hRecoTruth","Bpt",TCut(seldata_2y.Data()));
   ntGen->Project("hPtGen","Gpt",TCut(weight)*(TCut(selmcgen.Data())));
//   ntGen->Project("hPtGenAll","Gpt",TCut(weight)*(TCut(selmc.Data())));
  ...
  hPtCor->Draw();
    ...
    texCms->Draw();
  //hPtGen->Draw("same");
  //cCor->SaveAs(Form("ResultBs/PtCor_PbPb.pdf"));
  //cCor->SaveAs(Form("ResultBs/3K_PtCor_PbPb.pdf"));
  //pthat0//cCor->SaveAs(Form("ResultBs/PtCor_PbPb_MC.pdf"));
  cCor->SaveAs(Form("ResultBs/PtCor_PbPb_MC_pthat5.pdf"));
  //cCor->SaveAs(Form("ResultBs/PtCor_v3.pdf"));
  ...
  hPtSigma->Draw();
    ...
    texY->Draw();
    texCms->Draw();
  //cSigma->SaveAs(Form("ResultBs/PtSigma_PbPb.pdf"));
  //cSigma->SaveAs(Form("ResultBs/3K_PtSigma_PbPb.pdf"));
  //pthat0//cSigma->SaveAs(Form("ResultBs/PtSigma_PbPb_MC.pdf"));
  cSigma->SaveAs(Form("ResultBs/PtSigma_PbPb_MC_pthat5.pdf"));
  //cSigma->SaveAs(Form("ResultBs/PtSigma_v3.pdf"));
 ...
 //TFile *outf = new TFile("../ResultsBs/test_outputBs.root","recreate");
  //TFile *outf = new TFile("ResultBs/fitBs_PbPb.root","recreate");
  //TFile *outf = new TFile("ResultBs/fitBs_PbPb.root","recreate");
  //TFile *outf = new TFile("ResultBs/PbPb_MC.root","recreate");
  TFile *outf = new TFile("ResultBs/PbPb_MC_pthat5.root","recreate");
  //TFile *outf = new TFile("ResultBs/fitBs_v3.root","recreate");
  outf->cd();
  hPt->Write();
  hPtMC->Write();
  hEffAcc->Write();
...
//MC icin bu kismin tamamini kapa
  ...
  *//// Bkpi->Draw("same");
  ...
  // background->Draw("same");   
   ...
   TLegend* leg0 = myLegend(0.08,0.83,0.40,0.90);
   leg0->AddEntry(h,"CMS Internal","");
   leg0->Draw();
   //TLegend* leg1 = myLegend(0.11,0.78,0.52,0.82);
   TLegend* leg1 = myLegend(0.09,0.78,0.40,0.82);
   ...
   TLegend *leg3 = myLegend(0.40,0.50,0.60,0.60);
...
//   leg->AddEntry(Bkpi,"Non-prompt J/#psi","f");
   ...
   leg2->Draw();
*/
   //c->SaveAs(Form("ResultBs/BMass_PbPb-%d.pdf",count));
   //c->SaveAs(Form("ResultBs/3K_BMass_PbPb-%d.pdf",count));
   //pthat0//c->SaveAs(Form("ResultBs/BMass_PbPb_MC_%d.pdf",count));
   c->SaveAs(Form("ResultBs/BMass_PbPb_MC_pthat5_%d.pdf",count));
   //c->SaveAs(Form("ResultBs/BMass_v3-%d.pdf",count));
