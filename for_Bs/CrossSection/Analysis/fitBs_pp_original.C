...
double luminosity=28;//for 2015 pp 
...
//pp mc//
TString inputmc="/data/cdzn/Bsmeson2017/MC/Bntuple/pp/loop_Bs0_pthat5_bstojpsiphi_pp.root";
...
// if (doweight==0) weight="1";
  ...
  TLatex* texlumi = new TLatex(0.9,0.92," 28 pb^{-1} (5.03 TeV pp)");
  texlumi->SetNDC();
...
{   
   ...
   TString iNP="3.12764e1*Gaus(x,5.33166,3.64663e-2)*(x<5.33166)+(x>=5.33166)*3.12764e1*Gaus(x,5.33166,1.5204e-1)+2.11124e2*TMath::Erf(-(x-5.14397) /6.43194e-2) +2.11124e2";
    //TString iNP="3.39711e1*Gaus(x,5.35002e0,3.87952e-2)*(x<5.35002e0)+(x>=5.35002e    0)*3.39711e1*Gaus(x,5.35002e0,1.14232e-1)+2.16376e2*TMath::Erf(-(x-5.14189)/8.66243e-2) + 2.16376e2";
...
//   leg->AddEntry(Bkpi,"Non-prompt J/#psi","f");
   ...
   leg2->Draw();
*/
   c->SaveAs(Form("ResultBs/BMass_pp-%d.pdf",count));
