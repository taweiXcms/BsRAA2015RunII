...
void CrossSectionRatio(TString particle="Bplus")
{
  ...
  //TFile*filePPReference=new TFile("../fonllOutput_pp_Bs.root");  
  //TFile*filePPReference=new TFile("../fonllOutput_pp_Bs_y2p4.root");  
  TFile*filePPReference=new TFile("../../FONLL/Results/fonllOutput_pp_Bs_y2p4.root");  
  TGraphAsymmErrors*gaeBplusReference=(TGraphAsymmErrors*)filePPReference->Get(Form("gaeSigma%s",particle.Data()));
  TFile*filepPb=new TFile(Form("ResultBs/PtSigma%s.root",particle.Data()));
  ...
  TH2F* hemptySigma=new TH2F("hemptySigma","",50,5.,110,10.,1.1e1,1.e8);  
  ...
  hemptySigma->Draw();
  gaeBplusReference->Draw("5same");//FONLL
  hSigmapPbStat->Draw("epsame"); //Crosssection Bplus
  ...
  TLatex * tlatex1=new TLatex(0.1612903,0.8625793,"CMS Preliminary     pp #sqrt{s}= 5.03 TeV");
  tlatex1->SetNDC();
  ...
  TH2F* hemptyRatio=new TH2F("hemptyRatio","",50,5.,110,10.,0.07,1.9);
//  TH2F* hemptyRatio=new TH2F("hemptyRatio","",50,5.,80,10.,0.3,1.7);
