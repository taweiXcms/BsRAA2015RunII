...
int saveMasshisto(TString inputdata="",		// file name of input data
	...
	for(int i=0;i<varbins+1;i++) {
		Float_t ivar = varmin+(i-1)*varstep;

//		if (i==0) std::cout << setiosflags(ios::left) << "		- Processing varbin no cuts" << std::endl;
		if (ivar==varini) std::cout << setiosflags(ios::left) << "		- Processing varbin no cuts" << std::endl;
		...
		hMCSignal->Sumw2();
		TH1D* hMCSwapped = new TH1D("hMCSwapped","",binsmasshisto,minhisto,maxhisto);
		hMCSwapped->Sumw2();
